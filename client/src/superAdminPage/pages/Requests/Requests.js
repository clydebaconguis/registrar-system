...
import Confirm from "../../../adminPage/components/confirmPopUp/confirm";
import "./requests.css";
...
import DatePopUp from "../../../adminPage/components/releaseDatePopUp/datePopUp";
import ValidatePopUp from "../../../adminPage/components/claimPopUp/ClaimPopUp";
function Requests() {
  ...
  useEffect(() => {
    const combinedRequests = [...requestCFRD1, ...requestCFRD];
  ...
  const approveRequestTOR = async (id) => {
    try {
      const res = await axios.put(
        `http://localhost:3001/api/superadmin/request/CFRD/${id}`,
        {
          note: note,
        }
      );
  ...
  const updateTheRequest = async (message) => {
    try {
      const res = await axios.put(
        `http://localhost:3001/api/superadmin/request/holdApprovedRequest/${theID}`,
        {
          message: message,
          note: note,
        }
  ...
  const updateTheRequest1 = async (id, releaseDate) => {
    const date = new Date(releaseDate);
    try {
      const res = await axios.put(
        `http://localhost:3001/api/superadmin/request/holdApprovedRequest/${id}`,
        {
          releaseDate: date,
          note: note,
        }
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          "http://localhost:3001/api/superadmin/request/getRequestsCFRD"
        );
  ...
  return (
    <div className="admin-requests-container">
      ...
      {datePopUp && (
        <DatePopUp
          onSubmit={updateTheRequest1}
          onCancel={addReleaseDate}
          ID={theID}
        />
      )}
      <Sidebar
        ...
        >
          <div className="admin-requests-top" ref={myWidgetDiv}>
            <span className="admin-top-title">
              CLEARANCE FOR REQUISITION OF DOCUMENTS
            </span>
