...
import Solopage from "../components/torprofile/Solopage";
...
function Requests() {
  ...
  const updateTheRequest = async (id, message, releaseDate) => {
    try {
      const res = await axios.put(
        `http://localhost:3001/api/admin/request/holdApprovedRequest/${
          theID ? theID : id
        }`,
        {
          message: message,
  ...
  const updateTheRequest1 = async (id, releaseDate) => {
    ...
    try {
      const res = await axios.put(
        `http://localhost:3001/api/admin/request/holdApprovedRequest/${id}`,
        {
          releaseDate: date,
          note: note,
        }
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          "http://localhost:3001/api/admin/request/getRequestsCFRD"
        );
