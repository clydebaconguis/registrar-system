...
function RequestHD() {
  ...
  const approveRequestHD = async (id, type) => {
    try {
  ...
  const declineRequestHD = async (id, type) => {
    try {
      const res = await axios.put(
        `http://localhost:3001/api/signatories/request/CFRD/${id}`,
        {
        ...
        return (
          <div className="tableAction">
            ...
            <div
              ...
              onClick={() => approveRequestHD(params.row.id, 1)}
            >
              Approve
            </div>
            <div
              className="columnDecline"
              onClick={() => declineRequestHD(params.row.id, 2)}
            >
              Decline
            </div>
          </div>
      ...
      renderCell: (params) => {
        return (
          <div className="tableAction">
            ...
            <div
              className="columnApprove"
              onClick={() => approveRequestHD(params.row.id, 2)}
            >
              Approve
            </div>
          </div>
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          "http://localhost:3001/api/signatories/status/documentStatus/hd"
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(
          "http://localhost:3001/api/signatories/request/getAllRequestHD"
        );
        setAlldata(res.data);
      } catch (err) {
        console.log(err);
